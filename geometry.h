...
typedef std::vector<std::vector<float>> martix;

//impl rall
class zbuffer{
    public:
        std::unique_ptr<martix> z_buffer;

    zbuffer(int height,int width):z_buffer{new martix(width,std::vector<float>(height,-std::numeric_limits<float>::max()))}{};
